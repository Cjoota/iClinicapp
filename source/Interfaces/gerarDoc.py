        ...
        if nome and cpf and nascimento:
            ...
            for modelo in modelos_selecionados:
                ...
                if modelo == "ANAMNESE":
                    ...
                    ws["L14"] = self.empresas[1] # type: ignore
                    ws["L16"] = f"{idade} Anos"
                    ws["H11"] = tipo_exame[0]
                    saida = Path(r"documentos_gerados")
                    saida.mkdir(exist_ok=True)
            ...
            # REGISTRAR EXAMES - AQUI É ONDE SALVA NA PASTA RELACOES
            try:
                print(f"Tentando registrar exames para: {self.empresas[0]} - {nome}")
                self.controle.registrar_exames(self.empresas[0], self.empresas[1], nome, exames_selecionados, self.dataselect)
                print("Exames registrados com sucesso!")
                
                # Debug - verificar se o arquivo foi criado
                relacoes_path = Path("relacoes")
                print(f"Arquivos na pasta relacoes:")
                for arquivo in relacoes_path.glob("*.xlsx"):
                    print(f"  - {arquivo.name}")
                    
            except Exception as e:
                print(f"ERRO ao registrar exames: {e}")
                self.main.barra_aviso(f"Erro ao registrar exames: {str(e)}", ft.Colors.RED)
